...
### 4) 时间字段迁移（新增）
如果你要记录入场/出场时间，请在 Supabase 执行：
```sql
alter table public.trades
  add column if not exists "entryTime" text not null default '',
  add column if not exists "exitTime" text not null default '';
```
...
## OSS CORS 配置（上传报错时必看）
如果浏览器报错：`No Access-Control-Allow-Origin header`，需要在 OSS Bucket 控制台配置 CORS 规则。

建议规则（最小可用）：
- 来源（AllowedOrigin）：你的前端域名（例如 `https://trading-review-tool-xxx.vercel.app`）
- 方法（AllowedMethod）：`PUT, GET, HEAD, OPTIONS`
- 请求头（AllowedHeader）：`*`
- 返回头（ExposeHeader）：`ETag, x-oss-request-id`
- 预检缓存（MaxAgeSeconds）：`600`

> 若仅测试可临时放开 `AllowedOrigin=*`，上线请改成精确域名。
