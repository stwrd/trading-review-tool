...
export default function TradeJournalPage({ trades, activeUserId, onSave }: Props) {
  const emptyTrade: Omit<Trade, 'id'> = {
    ...
    entryPrice: 0, entryTime: '', stopLossPrice: 0, exitPrice: 0, targetPrice: 0, exitTime: '', rMultiple: 0, setupQualified: true, plannedEntry: true, plannedStop: true,
  ...
  return <div className="space-y-4">
    ...
    <Card>
      ...
      <p className="mb-3 text-xs text-slate-500">新手填写建议：先填基础信息和价格信息，平仓后补 R 倍数；“符合 setup”= 入场前就满足你定义的形态；情绪状态可写“平静/急躁/害怕错过”等。</p>
      <p className="mb-2 text-xs font-medium text-slate-600">基础信息</p>
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <label className="text-xs text-slate-500">日期（交易发生日）<input className="input mt-1" type="date" value={form.date} onChange={(e)=>setField('date',e.target.value)} /></label>
        <label className="text-xs text-slate-500">品种（如 BTC/黄金）<input className="input mt-1" placeholder="品种" value={form.symbol} onChange={(e)=>setField('symbol',e.target.value)} /></label>
        <label className="text-xs text-slate-500">周期（如 5m/1h）<input className="input mt-1" placeholder="周期" value={form.timeframe} onChange={(e)=>setField('timeframe',e.target.value)} /></label>
        <label className="text-xs text-slate-500">方向<select className="input mt-1" value={form.direction} onChange={(e)=>setField('direction',e.target.value as Trade['direction'])}><option>多</option><option>空</option></select></label>
        <label className="text-xs text-slate-500">Setup 类型（你的入场模型）<select className="input mt-1" value={form.setupType} onChange={(e)=>setField('setupType',e.target.value as SetupType)}>{setupTypes.map(v=><option key={v}>{v}</option>)}</select></label>
        <label className="text-xs text-slate-500">市场环境<select className="input mt-1" value={form.marketCondition} onChange={(e)=>setField('marketCondition',e.target.value as MarketCondition)}>{marketConditions.map(v=><option key={v}>{v}</option>)}</select></label>
        <label className="text-xs text-slate-500">R 倍数（结果）<input className="input mt-1" type="number" step="0.1" placeholder="例如 +1.2 / -0.6" title="R = (出场价-入场价)/每笔风险，盈利填正数，亏损填负数" value={form.rMultiple} onChange={(e)=>setField('rMultiple',Number(e.target.value))} /></label>
        <label className="text-xs text-slate-500">主要错误<select className="input mt-1" value={form.errorType} onChange={(e)=>setField('errorType',e.target.value as ErrorType)}>{errorTypes.map(v=><option key={v}>{v}</option>)}</select></label>
      </div>
      <p className="mt-3 mb-2 text-xs font-medium text-slate-600">价格信息（用于还原入场/止损/止盈）</p>
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <label className="text-xs text-slate-500">入场价<input className="input mt-1" type="number" step="0.0001" value={form.entryPrice} onChange={(e)=>setField('entryPrice',Number(e.target.value))} /></label>
        <label className="text-xs text-slate-500">入场时间（HH:mm）<input className="input mt-1" type="time" value={form.entryTime} onChange={(e)=>setField('entryTime',e.target.value)} /></label>
        <label className="text-xs text-slate-500">止损价<input className="input mt-1" type="number" step="0.0001" value={form.stopLossPrice} onChange={(e)=>setField('stopLossPrice',Number(e.target.value))} /></label>
        <label className="text-xs text-slate-500">目标价<input className="input mt-1" type="number" step="0.0001" value={form.targetPrice} onChange={(e)=>setField('targetPrice',Number(e.target.value))} /></label>
        <label className="text-xs text-slate-500">出场价<input className="input mt-1" type="number" step="0.0001" value={form.exitPrice} onChange={(e)=>setField('exitPrice',Number(e.target.value))} /></label>
        <label className="text-xs text-slate-500">出场时间（HH:mm）<input className="input mt-1" type="time" value={form.exitTime} onChange={(e)=>setField('exitTime',e.target.value)} /></label>
      </div>
      <p className="mt-3 mb-2 text-xs font-medium text-slate-600">执行检查（勾选表示“是”）</p>
      ...
      <p className="mt-3 mb-2 text-xs font-medium text-slate-600">复盘补充</p>
